...
int getans(int x, int k, int xl, int xr){
    ...
    if(k <= getsum(0, M, 0, 0, massive.size())){
        return getans(L, k, LL);
    } else{
        return getans(R, k, RR);
...
void solve(int ccase){
    ...
    repeat(q){
        ...
        if(type == 1){
            int k;get(k);
            if((k > getsum(0, n, 0, 0, n))){
                cout << "NO" << ENDL;
                continue;
            } 
            int ans = getans(0, k, 0, n);
            if(ans == -1){
                cout << "NO" << ENDL;
            } else{
                cout << ans << ENDL;
            }
            // cout << getsum(0, n, 0, 0, n) << ENDL;
        } else{
            int l, r;get(l, r);
