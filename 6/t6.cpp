...
node getmaxsum(int l, int r, int x, int xl, int xr){
    if(xl >= r or xr <= l) return node(INT_MIN / 2, INT_MIN / 2, INT_MIN / 2, INT_MIN / 2);
    if(xl >= l and xr <= r){
        return t[x];
    }
    if(d[x] != INT_MIN / 2){
...
void solve(int ccase){
    ...
    t = vector<node>(4 * (n + 1), node(INT_MIN / 2, INT_MIN / 2, INT_MIN / 2, INT_MIN / 2));
    ...
    repeat(q){
        ...
        if(l == 0){
            change(r - 1, r, v, 0, 0, n);
        } else{
            node a = getmaxsum(r - 1, v, 0, 0, n);
             cout << a.ans << ENDL;
        }
    }
