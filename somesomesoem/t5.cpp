...
int kmp(string s){
    ...
    // }
    print(prefixfunction); put(ENDL);
    // return 1;
    int shortest = s.size() - prefixfunction[s.size() - 1];
    for(int i = 0; i < s.size() - 1; i++){
        if(prefixfunction[i] == prefixfunction[s.size() - 1]){
            shortest = min(shortest, ll(s.size() - (i + 1ll)));
        }
    }
    return shortest;
}
